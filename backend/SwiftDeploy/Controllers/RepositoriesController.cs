...
//                var analysis = await analyzer.AnalyzeRepository(request.Owner, request.RepoName, request.Branch);
...
//            catch (NotFoundException)
//            {
//                return NotFound($"Repository '{request.Owner}/{request.RepoName}' not found or you do not have access.");
//            }
//            catch (Exception ex)
//            {
...
//                {
//                    success = false,
//                    error = $"An error occurred while analyzing the repository: {ex.Message}"
//                });
//            }
//        }
//    }
...
namespace SwiftDeploy.Controllers
{
    ...
    public class RepositoriesController : ControllerBase
    {
        ...
        /// <summary>
        /// Extracts and sets the GitHub access token from the GitHub-API-Key header.
        /// </summary>
        private bool TryAuthenticate(out string errorMessage)
        {
            errorMessage = string.Empty;
            ...
            Console.WriteLine($"=== GitHub Authentication Debug ===");
            Console.WriteLine($"GitHub-API-Key Header: {HttpContext.Request.Headers["GitHub-API-Key"].FirstOrDefault()}");
            Console.WriteLine($"Authorization Header: {HttpContext.Request.Headers["Authorization"].FirstOrDefault()}");
            Console.WriteLine($"Cookie Token: {HttpContext.Request.Cookies["GitHubAccessToken"]}");
            Console.WriteLine($"Final Token: {(string.IsNullOrEmpty(githubToken) ? "NOT FOUND" : "FOUND")}");

            if (string.IsNullOrEmpty(githubToken))
            {
                errorMessage = "Missing GitHub token. Please provide via GitHub-API-Key header, Authorization header, or GitHubAccessToken cookie.";
                Console.WriteLine("ERROR: No GitHub token found in any source");
                return false;
            }
            ...
            if (string.IsNullOrEmpty(githubToken))
            {
                errorMessage = "GitHub token is empty after trimming.";
                Console.WriteLine("ERROR: GitHub token is empty after trim");
                return false;
            }
            ...
            try
            {
                ...
                Console.WriteLine($"SUCCESS: GitHub credentials set (token length: {githubToken.Length}, starts with: {githubToken.Substring(0, Math.Min(7, githubToken.Length))}...)");
                return true;
            }
            catch (Exception ex)
            {
        ...
        // FIXED: Changed route to avoid conflict
